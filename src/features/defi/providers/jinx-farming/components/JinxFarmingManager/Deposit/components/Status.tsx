...
export const Status: React.FC<StatusProps> = ({ accountId }) => {
  ...
  if (!asset)
    throw new Error(`Asset not found for AssetId ${jinxFarmingOpportunity?.underlyingAssetId}`)
  if (!feeAsset) throw new Error(`Fee asset not found for AssetId ${feeAssetId}`)
  ...
  useEffect(() => {
    if (!jinxFarmingOpportunity) return
  ...
  if (!state || !dispatch || !jinxFarmingOpportunity) return null
      ...
      case 'success':
        return {
          ...
          statusBody: translate('modals.deposit.status.success', {
            opportunity: jinxFarmingOpportunity?.opportunityName,
          }),
          statusBg: 'green.500',
        }
      case 'failed':
  ...
  return (
    <TxStatus
      ...
      pairIcons={jinxFarmingOpportunity?.icons}
    >
      <Summary>
