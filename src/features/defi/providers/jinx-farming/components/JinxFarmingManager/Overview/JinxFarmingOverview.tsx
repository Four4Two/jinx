...
import { JinxFarmingEmpty } from './JinxFarmingEmpty'
import { WithdrawCard } from './WithdrawCard'

type JinxFarmingOverviewProps = {
  accountId?: AccountId | undefined
  onAccountIdChange: AccountDropdownProps['onChange']
}

export const JinxFarmingOverview: React.FC<JinxFarmingOverviewProps> = ({
  accountId,
  ...
  const lpAsset = assets[jinxEthLpAssetId]
  ...
  const underlyingAssetsFiatBalance = useMemo(() => {
    ...
    const jinxEthLpFiatPrice = marketData?.[opportunityData?.underlyingAssetId ?? '']?.price ?? '0'
    return bnOrZero(cryptoAmount).times(jinxEthLpFiatPrice).toString()
  }, [
    marketData,
