...
export const JinxyDeposit: React.FC<{
  ...
  const {
    chainId,
    contractAddress: jinxyContractAddress,
  ...
  useEffect(() => {
    ;(async () => {
      try {
        ...
        dispatch({
          ...
          payload: { ...jinxyOpportunity, apy: jinxyAprData?.jinxyApr ?? '' },
        })
      } catch (error) {
    ...
    bip44Params,
    chainAdapterManager,
    jinxyContractAddress,
    walletState.wallet,
    jinxyAprData?.jinxyApr,
    isJinxyAprLoading,
    jinxyStakingContractAddress,
  ])
  ...
  }, [accountId, handleAccountIdChange, jinxyContractAddress, translate, stakingAsset.symbol])
