...
export const Confirm: React.FC<StepComponentProps & { accountId?: AccountId | undefined }> = ({
  ...
  const withdrawalFee = useMemo(() => {
    return state?.withdraw.withdrawType === WithdrawType.INSTANT
      ? bnOrZero(
          bn(state?.withdraw.cryptoAmount ?? '0').times(state?.jinxyFeePercentage ?? '0'),
        ).toString()
      : '0'
  }, [state?.withdraw.withdrawType, state?.withdraw.cryptoAmount, state?.jinxyFeePercentage])
  ...
  const handleConfirm = useCallback(async () => {
    try {
      if (
        ...
        return
      dispatch({ type: JinxyWithdrawActionType.SET_LOADING, payload: true })

      if (!supportsETH(walletState.wallet))
        throw new Error(`handleConfirm: wallet does not support ethereum`)

      const txid = await jinxyApi.withdraw({
      ...
      dispatch({ type: JinxyWithdrawActionType.SET_TXID, payload: txid })
      ...
      dispatch({ type: JinxyWithdrawActionType.SET_LOADING, payload: false })
    } catch (error) {
      console.error(error)
    }
  }, [
