...
import { ASSET_REFERENCE, toAssetId } from '@sudophunk/caip'
...
export const ClaimStatus: React.FC<ClaimStatusProps> = ({ accountId }) => {
  ...
  const refetchJinxyBalances = useCallback(() => {
    if (!accountId) return
  ...
  }, [refetchJinxyBalances, estimatedGas, jinxyApi, state, txid, poll])
