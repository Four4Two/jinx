...
import { ASSET_NAMESPACE, ASSET_REFERENCE, toAssetId } from '@sudophunk/caip'
...
export const ClaimConfirm = ({
  ...
  // The highest level AssetId/OpportunityId, in this case of the single JINXy contract
  ...
  const handleConfirm = useCallback(async () => {
    if (!(walletState.wallet && contractAddress && userAddress && jinxyApi && bip44Params)) return
    setLoading(true)
    try {
      if (!supportsETH(walletState.wallet))
        throw new Error(`handleConfirm: wallet does not support ethereum`)
      const txid = await jinxyApi.claimWithdraw({
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (!(walletState.wallet && contractAddress && jinxyApi && chainAdapter)) return
        if (!supportsETH(walletState.wallet))
        ...
        const feeDataEstimate = await jinxyApi.estimateClaimWithdrawFees({
          claimAddress: userAddress,
          userAddress,
          contractAddress,
          wallet: walletState.wallet,
