...
type JinxEthProviderProps = {
  children: React.ReactNode
}

type IJinxEthContext = {
...
export const JinxEthProvider = ({ children }: JinxEthProviderProps) => {
  ...
  const refetchJinxEthStakingAccountData = useCallback(async () => {
    await Promise.all(
  ...
  useEffect(() => {
    if (farmingAccountId && transaction && transaction.status !== TxStatus.Pending) {
      if (transaction.status === TxStatus.Confirmed) {
        refetchJinxEthStakingAccountData()
  ...
  return <JinxEthContext.Provider value={value}>{children}</JinxEthContext.Provider>
}

export const useJinxEth = () => useContext(JinxEthContext)
