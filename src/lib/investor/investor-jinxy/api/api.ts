...
import { jinxyAbi } from '../abi/jinxy-abi'
import { jinxyStakingAbi } from '../abi/jinxy-staking-abi'
...
export type ConstructorArgs = {
  ...
  providerUrl: string
  jinxyAddresses: JinxyAddressesType
...
export const transformData = ({
  ...
  return {
    ...
    rewardToken: contractData.jinxy,
    stakingToken: contractData.jinx,
...
export class JinxyApi {
  ...
  constructor({
    ...
    this.jinxyStakingContracts = jinxyAddresses.map(
      addresses => new ethers.Contract(addresses.staking, jinxyStakingAbi, this.provider),
    )
    this.liquidityReserveContracts = jinxyAddresses.map(
      addresses =>
    ...
    this.jinxyAddresses = jinxyAddresses
  }
  ...
  private getStakingContract(contractAddress: string): ethers.Contract {
    const stakingContract = this.jinxyStakingContracts.find(
  ...
  async getJinxyOpportunities() {
    try {
      const opportunities = await Promise.all(
        this.jinxyAddresses.map(async addresses => {
          const stakingContract = this.jinxyStakingContracts.find(
            item => toLower(item.address) === toLower(addresses.staking),
          )
          try {
            const expired = await stakingContract?.pauseStaking()
            const tvl = await this.tvl({ tokenContractAddress: addresses.jinxy })
            const apy = this.apy()
      ...
      throw new Error(`getJinxyOpportunities Error: ${e}`)
    }
  }

  async getJinxyOpportunityByStakingAddress(stakingAddress: string) {
    this.verifyAddresses([stakingAddress])
    const addresses = this.jinxyAddresses.find(item => {
  ...
  async tvl(input: TokenAddressInput): Promise<BigNumber> {
    ...
    this.verifyAddresses([tokenContractAddress])
    const contract = new ethers.Contract(tokenContractAddress, jinxyAbi, this.provider)
  ...
  async getRebaseHistory(input: BalanceInput) {
    ...
    const jinxyContract = new ethers.Contract(tokenContractAddress, jinxyAbi, this.provider)
    const fromBlock = 14381454 // genesis rebase

    const rebaseEvents = await (async () => {
      try {
        const filter = jinxyContract.filters.LogRebase()
        const events = await jinxyContract.queryFilter(filter, fromBlock, 'latest')
    ...
    const transferEvents = await (async () => {
      try {
        const filter = jinxyContract.filters.Transfer()
        const events = await jinxyContract.queryFilter(filter, fromBlock, 'latest')
        return events
      } catch (e) {
      ...
      events.map(async event => {
        const { preRebaseBalance, postRebaseBalance } = await (async () => {
          try {
            ...
            const postRebaseBalanceResult = await jinxyContract.balanceOf(userAddress, {
              blockTag: event.blockNumber,
            })
            const unadjustedPreRebaseBalance = await jinxyContract.balanceOf(userAddress, {
    ...
    const actualResults = results.reduce<RebaseHistory[]>((acc, cur) => {
      if (cur.status === 'rejected') {
        console.error('getJinxyRebaseHistory: balanceOf call failed - charts will be wrong')
        return acc
      }
