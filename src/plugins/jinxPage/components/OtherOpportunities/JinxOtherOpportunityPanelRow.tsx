...
import type { ExternalOpportunity } from '../../JinxCommon'

type JinxOtherOpportunityPanelRowProps = {
  opportunity: ExternalOpportunity
}

export const JinxOtherOpportunityPanelRow: React.FC<JinxOtherOpportunityPanelRowProps> = ({
  opportunity,
}) => {
  const {
  ...
  const opportunityButtonTranslation = useMemo(() => {
    if (opportunity.link) return 'plugins.jinxPage.getStarted'
    if (isDemoWallet || !wallet || !supportsETH(wallet)) return 'common.connectWallet'

    return hasActivePosition ? 'plugins.jinxPage.manage' : 'plugins.jinxPage.getStarted'
  ...
  return (
    ...
    >
      ...
      <Skeleton isLoaded={Boolean(earnOpportunity)} textAlign={{ base: 'right', md: 'center' }}>
        <Box>
          <Text translation='plugins.jinxPage.currentApy' color='text.subtle' mb={1} />
          <Box
