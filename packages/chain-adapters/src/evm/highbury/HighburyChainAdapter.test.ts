...
import * as highbury from './HighburyChainAdapter'
...
describe('HighburyChainAdapter', () => {
  ...
  const makeChainAdapterArgs = (overrideArgs?: {
    providers?: { http: unchained.highbury.V1Api }
    chainId?: EvmChainId
  }): ChainAdapterArgs<unchained.highbury.V1Api> =>
    merge(
      {
        providers: {
          http: {} as unknown as unchained.highbury.V1Api,
          ws: {} as unchained.ws.Client<unchained.highbury.Tx>,
        },
        rpcUrl: '',
      },
      overrideArgs,
  ...
  describe('constructor', () => {
    it('should return chainAdapter with Highbury mainnet chainId if called with no chainId', () => {
      const args = makeChainAdapterArgs()
      const adapter = new highbury.ChainAdapter(args)
      const chainId = adapter.getChainId()
      expect(chainId).toEqual(highburyChainId)
    })

    it('should return chainAdapter with valid chainId if called with valid chainId', () => {
      const args = makeChainAdapterArgs({ chainId: KnownChainIds.HighburyMainnet })
      const adapter = new highbury.ChainAdapter(args)
      const chainId = adapter.getChainId()
      expect(chainId).toEqual(highburyChainId)
    })
  ...
  describe('getFeeAssetId', () => {
    it('should return the correct fee assetId', () => {
      const adapter = new highbury.ChainAdapter(makeChainAdapterArgs())
      expect(adapter.getFeeAssetId()).toEqual(highburyAssetId)
    })
  })
  ...
  describe('getFeeData', () => {
    it('should return current network fees', async () => {
      ...
      } as unknown as unchained.highbury.V1Api
    ...
    it('should sign a properly formatted txToSign object', async () => {
      ...
      const tx = {
        ...
        txToSign: {
          ...
          value: '0x0',
          to: EOA_ADDRESS,
          chainId: Number(CHAIN_REFERENCE.HighburyMainnet),
  ...
  describe('buildSendTransaction', () => {
    it('should throw if passed tx has no "to" property', async () => {
      ...
      } as unknown as BuildSendTxInput<KnownChainIds.HighburyMainnet>
  ...
  describe('getBIP44Params', () => {
    ...
    it('should return the correct coinType', () => {
      ...
      expect(result.coinType).toStrictEqual(Number(ASSET_REFERENCE.Highbury))
    })

    it('should respect accountNumber', () => {
      const testCases: BIP44Params[] = [
        { purpose: 44, coinType: Number(ASSET_REFERENCE.Highbury), accountNumber: 0 },
        { purpose: 44, coinType: Number(ASSET_REFERENCE.Highbury), accountNumber: 1 },
        { purpose: 44, coinType: Number(ASSET_REFERENCE.Highbury), accountNumber: 2 },
      ]
